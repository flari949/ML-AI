...
net_save_name = save_name + '_cnn2_net.h5'
checkpoint_save_name = save_name + '_cnn2_net.chk'
history_save_name = save_name + '_cnn2_net.hist'
...
all_train_labels = np.concatenate((train_labels[:, 3], train_labels[:, 3]))
...
if load_from_file and os.path.isfile(net_save_name):
    ...
    net.add(layers.MaxPooling2D(pool_size=2))
    net.add(layers.Conv2D(filters=256, kernel_size=3, activation='relu', padding='valid'))
    net.add(layers.MaxPooling2D(pool_size=2))
    ...
    train_info = net.fit(data_train_aug, validation_data=(x_val, y_val), epochs=150, batch_size=16,
    ...
    full_test_labels = np.concatenate((test_labels[:, 3], test_labels[:, 3]))

    test_mse_score, test_mae_score = net.evaluate(full_test_set, full_test_labels)
    print(test_mae_score)

    # Plot training and validation accuracy and loss progression over time
    acc = train_info.history['mae']
    val_acc = train_info.history['val_mae']
